...
import { useToastError } from '@/src/components/Toast';
import { AVAILABLE_LANGUAGES } from '@/src/constants/i18next';
import { useCreateAccount } from '@/src/services/account/account.service';
            ...
            <Stack spacing="4">
              ...
              <FieldInput
                ...
                validations={[
                  {
                    rule: isMinLength(2),
                    message: t('account:data.login.tooShort', { min: 2 }) as string,
                  },
                  {
                    rule: isMaxLength(50),
                    message: t('account:data.login.tooLong', { max: 50 }) as string,
                  },
                  {
                    rule: isPattern(
                      '^[a-zA-Z0-9!$&*+=?^_`{|}~.-]+@[a-zA-Z0-9-]+(?:\\.[a-zA-Z0-9-]+)*$|^[_.@A-Za-z0-9-]+$'
                    ),
                    message: t('account:data.login.invalid') as string,
                  },
                ]}
              />
              <FieldInput
                ...
                validations={[
                  {
                    rule: isMinLength(5),
                    message: t('account:data.email.tooShort', { min: 5 }) as string,
                  },
                  {
                    rule: isMaxLength(254),
                    message: t('account:data.email.tooLong', { min: 254 }) as string,
                  },
                  {
                    rule: isEmail(),
              ...
              <FieldInput
                ...
                validations={[
                  {
                    rule: isMinLength(4),
                    message: t('account:data.password.tooShort', { min: 4 }) as string,
                  },
                  {
                    rule: isMaxLength(50),
                    message: t('account:data.password.tooLong', { min: 50 }) as string,
                  },
                ]}
              />
              <Flex>
